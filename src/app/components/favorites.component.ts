...
@Component({
  ...
  template: `
    <div>
      <button (click)="openFavoritesModal()" class="relative cursor-pointer focus-visible:outline-offset-2 focus-visible:outline-pink-500">
            ...
            @if (favorites().length > 0) {
              @for (fav of favorites(); track fav.title) {
                <div class="flex items-center gap-4 mb-3 p-3 rounded-lg bg-gray-50 shadow-sm border border-gray-200 transition hover:shadow-md">
                  ...
                  <button (click)="removeFavorite(fav)" class="p-2 rounded-full bg-red-100 text-pink-500 hover:bg-pink-600 hover:text-white transition-colors shadow-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pink-500 cursor-pointer" aria-label="Remove from favorites">
